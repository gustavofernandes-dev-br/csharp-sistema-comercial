...
{
    public partial class Indicacao : Form
    {
        public Indicacao()
        {
            InitializeComponent();
        ...
        public void alteraContato()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "UPDATE CLIENTES_INDICACAO SET [IND_NOME] = '" + txtNome.Text + "',[IND_ATUALIZACAO] = GETDATE(),[IND_USUARIO] = 1 ,[IND_INATIVO] = " + ativo + " where IND_codigo = '" + txtID.Text + "'";
                cmd.ExecuteNonQuery();
        ...
        public void incluiContato()
        {
            ...
            SqlCommand cmd = new SqlCommand();
            cmd.Connection = ConexaoBanco.conn;
            cmd.CommandText = "insert into CLIENTES_INDICACAO ([IND_CODIGO],[IND_NOME],[IND_CADASTRO],[IND_ATUALIZACAO] ,[IND_USUARIO] ,[IND_INATIVO]) values (@param1,@param2,@param3,@param4,@param5,@param6)";
        ...
        private void excluiRegistro()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = ConexaoBanco.conn;
                cmd.CommandText = "UPDATE CLIENTES_INDICACAO SET [IND_INATIVO] = '1' where IND_CODIGO =  " + txtID.Text + "";
        ...
        public void Load_DGV()
        {
            ...
            string usuarios = "SELECT [IND_CODIGO],[IND_NOME],[IND_CADASTRO],[IND_ATUALIZACAO],[IND_USUARIO],replace(replace([IND_INATIVO] ,'1','Inativo'),'0','Ativo') as IND_INATIVO  FROM [dbo].[CLIENTES_INDICACAO]  where IND_INATIVO = '0'";
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    dgvRegistros.Rows[i].Cells[0].Value = rw["IND_CODIGO"].ToString();
                    dgvRegistros.Rows[i].Cells[1].Value = rw["IND_NOME"].ToString();
                    dgvRegistros.Rows[i].Cells[2].Value = rw["IND_CADASTRO"].ToString();
                    dgvRegistros.Rows[i].Cells[3].Value = rw["IND_ATUALIZACAO"].ToString();
                    dgvRegistros.Rows[i].Cells[4].Value = rw["IND_USUARIO"].ToString();
                    dgvRegistros.Rows[i].Cells[5].Value = rw["IND_INATIVO"].ToString();
        ...
        public bool verificaNomeRepetido()
        {
            ...
            {
                result = true;
                string usuario = "select * from CLIENTES_INDICACAO where rtrim(ltrim(IND_nome)) = '" + txtNome.Text + "'";
        ...
        public int idUsuario()
        {
            ...
            try
            {
                ...
                SqlCommand sql = new SqlCommand("Select resultado = CASE WHEN MAX(IND_CODIGO) is null then '1' else MAX(IND_CODIGO) + 1 end From CLIENTES_INDICACAO", ConexaoBanco.conn);
        ...
        private void Indicacao_Load(object sender, EventArgs e)
        {
            txtID.Enabled = false;
            txtNome.Enabled = false;
            gbxDadosCadastrais.Enabled = false;
        ...
        private void bntPesquisar_Click(object sender, EventArgs e)
        {
            ...
            { pesquisarRegistro = " and ind_nome like '%" + txtNomePesquisar.Text + "%'"; }
            else if (txtNomePesquisar.Text.Length > 0)
            { pesquisarRegistro = " and ind_nome like '" + txtNomePesquisar.Text + "%'"; }
            else
            { pesquisarRegistro = " "; }

            string usuarios = "SELECT [ind_CODIGO],[ind_NOME],[ind_CADASTRO],[ind_ATUALIZACAO],[ind_USUARIO], replace(replace(ind_INATIVO,'1','Inativo'),'0','Ativo') AS ind_INATIVO FROM [dbo].[CLIENTES_indicacao]  where ind_inativo = " + ativo + pesquisarRegistro + "  ";
        ...
        private void txtNomePesquisar_TextChanged_1(object sender, EventArgs e)
        {
            if (txtNomePesquisar.Text.Length > 0)
            {
                chkQualquerParte.Visible = true;
