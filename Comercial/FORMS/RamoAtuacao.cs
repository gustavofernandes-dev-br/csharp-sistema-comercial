...
{
    public partial class formRamoAtuacao : Form
    {
        public formRamoAtuacao()
        {
            InitializeComponent();
        }
        enum Acoes
        ...
        public void alteraContato()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "UPDATE CLIENTES_RAMOATUACAO SET [RAM_NOME] = '"+txtNome.Text+"',[RAM_ATUALIZACAO] = GETDATE(),[RAM_USUARIO] = 1 ,[RAM_INATIVO] = "+ativo+" where ram_codigo = '"+txtID.Text+"'";
                cmd.ExecuteNonQuery();  
        ...
        public void incluiContato()
        {
            ...
            if (chkInativo.Checked == true)
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = ConexaoBanco.conn;
                cmd.CommandText = "insert into CLIENTES_RAMOATUACAO ([RAM_CODIGO],[RAM_NOME],[RAM_CADASTRO],[RAM_ATUALIZACAO] ,[RAM_USUARIO] ,[RAM_INATIVO]) values (@param1,@param2,@param3,@param4,@param5,@param6)";
        ...
        private void excluiRegistro()
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand();
                cmd.Connection = ConexaoBanco.conn;
                cmd.CommandText = "UPDATE CLIENTES_RAMOATUACAO SET [RAM_INATIVO] = '1' where RAM_CODIGO =  " + txtID.Text + "";
        ...
        public void Load_DGV()
        {
            ...
            string usuarios = "SELECT [RAM_CODIGO],[RAM_NOME],[RAM_CADASTRO],[RAM_ATUALIZACAO],[RAM_USUARIO],replace(replace([RAM_INATIVO] ,'1','Inativo'),'0','Ativo') as RAM_INATIVO  FROM [dbo].[CLIENTES_RAMOATUACAO]  where RAM_INATIVO = '0'";
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                {
                    dgvRegistros.Rows[i].Cells[0].Value = rw["RAM_CODIGO"].ToString();
                    dgvRegistros.Rows[i].Cells[1].Value = rw["RAM_NOME"].ToString();
                    dgvRegistros.Rows[i].Cells[2].Value = rw["RAM_CADASTRO"].ToString();
                    dgvRegistros.Rows[i].Cells[3].Value = rw["RAM_ATUALIZACAO"].ToString();
                    dgvRegistros.Rows[i].Cells[4].Value = rw["RAM_USUARIO"].ToString();
                    dgvRegistros.Rows[i].Cells[5].Value = rw["RAM_INATIVO"].ToString();
        ...
        public bool verificaNomeRepetido()
        {
            ...
            {
                result = true;
                string usuario = "select * from CLIENTES_RAMOATUACAO where rtrim(ltrim(ram_nome)) = '" + txtNome.Text + "'";
        ...
        public int idUsuario()
        {
            ...
            try
            {
                SqlCommand sql = new SqlCommand("Select resultado = CASE WHEN MAX(RAM_CODIGO) is null then '1' else MAX(RAM_CODIGO) + 1 end From CLIENTES_RAMOATUACAO", ConexaoBanco.conn);
        ...
        private void formRamoAtuacao_Load(object sender, EventArgs e)
        {
        ...
        private void bntPesquisar_Click(object sender, EventArgs e)
        {
            ...
            { pesquisarRegistro = " and RAM_nome like '%" + txtNomePesquisar.Text + "%'"; }
            ...
            string usuarios = "SELECT [RAM_CODIGO],[RAM_NOME],[RAM_CADASTRO],[RAM_ATUALIZACAO],[RAM_USUARIO], replace(replace([RAM_INATIVO],'1','Inativo'),'0','Ativo') AS RAM_INATIVO FROM [dbo].[CLIENTES_RAMOATUACAO]  where ram_inativo = " + ativo + pesquisarRegistro + "  ";
